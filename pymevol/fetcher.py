...
class PyPIFetcher:
    ...
    def get_version_info(self, package_name: str, version: str) -> Optional[VersionInfo]:
        ...
        try:
            ...
            if version not in releases:
                logger.warning(f"Version {version} not found for package {package_name}")
                return None
            ...
            if version_data:
                ...
                if upload_time:
                    try:
                        ...
                        logger.warning(f"Could not parse upload_time '{upload_time}' for {package_name} {version}: {e}")
                        release_date = None
    ...
    def download_and_extract_version(self, package_name: str, version: str, 
                                   prefer_wheel: bool = True) -> Optional[Tuple[Path, VersionInfo]]:
