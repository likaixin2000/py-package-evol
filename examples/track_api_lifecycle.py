...
def track_api_lifecycle(package_name, api_name):
    ...
    try:
        ...
        print(f"  Introduced in: {lifecycle['introduced_in'] or 'Unknown (before first analyzed version)'}")
        ...
        if related_changes:
            ...
            for change in sorted(related_changes, key=lambda x: x.to_version or ''):
                print(f"    - {change.to_version}: {change.change_type.value.upper()} - {change.description}")
