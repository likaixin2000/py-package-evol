    ...
    try:
        ...
        if lifecycle['modifications']:
            ...
            for mod in lifecycle['modifications']:
                click.echo(f"    - Version {mod['version']}: {mod['description'] or 'Signature changed'}")
...
@click.option('--type', 'api_type', type=click.Choice(['function', 'class', 'method', 'property', 'constant']),
...
def list_apis(ctx, package, version, api_type, private):
    ...
    try:
        if version:
            ...
            if not version_info:
                click.echo(f"Version {version} not found for package {package}", err=True)
                return
...
def _save_results(results, output_path, format_type):
    ...
    if len(results) == 1:
        ...
        if format_type == 'json':
            ...
            generator = ReportGenerator()
            content = generator.generate_single_package_report(result.to_dict(), format_type)
            if not output_path.suffix:
