...
class PackageAnalyzer:
            ...
            for version_info in version_infos:
                ...
                try:
                    ...
                    if elements is not None:
                        ...
                        logger.info(f"Found {len(elements)} API elements in version {version_info.version}")
                    else:
                        logger.warning(f"Failed to analyze version {version_info.version}")
    ...
    def _analyze_version(self, package_name: str, version_info: VersionInfo) -> Optional[List[APIElement]]:
        ...
        try:
            ...
            if not package_dir:
                logger.warning(f"Could not find package directory for {package_name} {version_info.version}")
                return []
    ...
    def _find_package_directory(self, extracted_path: Path, package_name: str) -> Optional[Path]:
        ...
        for item in extracted_path.rglob('*'):
            if item.is_dir() and item.name in [package_name, package_name.replace('-', '_'), package_name.replace('_', '-')]:
    ...
    def _calculate_changes(self, versions: List[VersionInfo], 
                          api_elements: Dict[str, List[APIElement]]) -> List[APIChange]:
        ...
        for i in range(len(sorted_versions)):
            ...
            current_apis = {self._get_api_key(api): api for api in api_elements.get(current_version, [])}
            ...
            if i == 0:
                ...
                previous_apis = {self._get_api_key(api): api for api in api_elements.get(previous_version, [])}
