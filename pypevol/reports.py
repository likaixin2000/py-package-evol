...
class ReportGenerator:
    ...
    def generate_single_package_report(self, result_data: Dict[str, Any], format_type: str) -> str:
    ...
    def generate_multi_package_report(self, results_data: Dict[str, Dict[str, Any]], 
                                    format_type: str) -> str:
...
'''
        ...
        for change in sorted(changes, key=lambda x: x.get('to_version', ''), reverse=True)[:20]:
...
'''
        ...
        for change in sorted(changes, key=lambda x: x.get('to_version', ''), reverse=True)[:20]:
            ...
            markdown_content += f"| {version} | {change_type} | `{api_name}` | {description} |\\n"
...
'''
        ...
        for version in sorted(versions, key=lambda x: x.get('release_date', ''), reverse=True):
    ...
    def _generate_csv_report(self, result_data: Dict[str, Any]) -> str:
        ...
        for change in changes:
            ...
            old_sig = change.get('old_signature', '').replace(',', ';') if change.get('old_signature') else ''
            new_sig = change.get('new_signature', '').replace(',', ';') if change.get('new_signature') else ''
            
            csv_content += f'"{version}","{change_type}","{api_type}","{api_name}","{module_path}","{description}","{old_sig}","{new_sig}"\\n'
        
        return csv_content
    
    def _generate_multi_markdown_report(self, results_data: Dict[str, Dict[str, Any]]) -> str:
...
'''
    ...
    def _generate_multi_html_report(self, results_data: Dict[str, Dict[str, Any]]) -> str:
...
'''
    ...
    def _generate_version_timeline_data(self, versions: List[Dict], changes: List[Dict]) -> Dict:
        ...
        for change in changes:
            ...
            if version not in changes_by_version:
                changes_by_version[version] = {'added': 0, 'removed': 0, 'modified': 0, 'deprecated': 0}
            changes_by_version[version][change['change_type']] += 1
        
        versions_list = [v['version'] for v in sorted(versions, key=lambda x: x.get('release_date', ''))]
        
        traces = []
        for change_type in ['added', 'removed', 'modified', 'deprecated']:
            y_values = [changes_by_version.get(v, {}).get(change_type, 0) for v in versions_list]
            traces.append({
                'x': versions_list,
                'y': y_values,
    ...
    def _generate_api_evolution_data(self, result_data: Dict) -> Dict:
        ...
        for api in api_list:
            ...
            for version in versions:
                present = any(e['full_name'] == api for e in api_elements.get(version, []))
